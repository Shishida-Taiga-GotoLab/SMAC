...
module subprogs
    ...
    ! 予測速度を計算(1ステップ目)
    subroutine prespeed_cal_explicit(Ax, Ay, Bx, By, u, u_pre, v, v_pre, dpdx, dpdy, p, Fx, Fy)
        real(8), intent(inout) :: Ax(1:Nx, 1:Ny), Ay(1:Nx, 1:Ny), Bx(1:Nx, 1:Ny), By(1:Nx, 1:Ny)
        ...
        do j = 1, Ny
            do i = 1, Nx
                ...
                u_pre(i, j) = u(i, j) + dt * (Ax(i, j) + Bx(i, j) - dpdx(i, j) + Fx(i, j))
            enddo
        enddo
        ...
        do j = 1, Ny
            do i = 1, Nx
                ...
                v_pre(i, j) = v(i, j) + dt * (Ay(i, j) + By(i, j) - dpdy(i, j) + Fy(i, j))
            enddo
        enddo

        

    end subroutine prespeed_cal_explicit


    ! 予測速度を計算(2ステップ目以降)
    subroutine prespeed_cal_implicit(Ax, Ay, Ax_pre, Ay_pre, Bx, By, u, u_pre, v, v_pre, dpdx, dpdy, p, Fx, Fy)
        real(8), intent(inout) :: Ax(1:Nx, 1:Ny), Ay(1:Nx, 1:Ny), Ax_pre(1:Nx, 1:Ny), Ay_pre(1:Nx, 1:Ny)
        real(8), intent(inout) :: Bx(1:Nx, 1:Ny), By(1:Nx, 1:Ny)
        real(8), intent(out) :: dpdx(0:Nx+1, 0:Ny+1), dpdy(0:Nx+1, 0:Ny+1)
        real(8), intent(out) :: u_pre(0:Nx+1, 0:Ny+1), v_pre(0:Nx+1, 0:Ny+1)
        real(8), intent(in) :: u(0:Nx+1, 0:Ny+1), v(0:Nx+1, 0:Ny+1), p(0:Nx+1, 0:Ny+1)
        real(8), intent(in) :: Fx(1:Nx, 1:Ny), Fy(1:Nx, 1:Ny)
        integer :: i, j, itr, itr_max
        real(8) :: er, er0, omg, RHS_u(1:Nx, 1:Ny), RHS_v(1:Nx, 1:Ny) 
        real(8) :: E_u(1:Nx, 1:Ny), E_v(1:Nx, 1:Ny)
        real(8) :: AXP, AXM, AYP, AYM, A0
        ...
        ! 差分式　定数項
        AXP = - (mu * dt) / (2.0d0 * dx**2)
        AXM = - (mu * dt) / (2.0d0 * dx**2)
        AYP = - (mu * dt) / (2.0d0 * dy**2)
        AYM = - (mu * dt) / (2.0d0 * dy**2)
        A0 =  -(1.0d0 + (mu * dt) * (1.0d0/(dx**2) + 1.0d0/(dy**2)))
        ...
        call convection_cal(u, v, Ax, Ay) ! 対流項を更新
        call viscosity_cal(u, v, Bx, By) ! 粘性項を更新
        ...
        do itr = 1, itr_max
            ...
            do j = 1, Ny
                do i = 1, Nx
                    ...
                    RHS_u(i, j) = u(i, j) + &
                    & dt * ((-dpdx(i, j) + Fx(i, j)) + (mu/2)*(BYM*u(i,j-1) + BXM*u(i-1,j) - B0*u(i, j)  + BXP*u(i+1,j) + BYP*u(i,j+1)) + (3.0d0*Ax(i, j) - Ax_pre(i, j))/2.0d0)

                    E_u(i, j) = AYM*u_pre(i,j-1) + AXM*u_pre(i-1,j) - A0*u_pre(i, j) + AXP*u_pre(i+1,j) + AYP*u_pre(i,j+1) - RHS_u(i, j)
                    u_pre(i, j) = u_pre(i, j) + omg * E_u(i, j) / A0
                    er = er + sqrt(E_u(i, j) ** 2)
                enddo
            enddo
        ...
        do itr = 1, itr_max
            ...
            do j = 1, Ny
                do i = 1, Nx
                    ...
                    RHS_v(i, j) = v(i, j) + &
                    & dt * ((- dpdy(i, j) + Fy(i, j)) + (mu/2)*(BYM*v(i,j-1) + BXM*v(i-1,j) - B0*v(i, j)  + BXP*v(i+1,j) + BYP*v(i,j+1)) + (3.0d0*Ay(i, j) - Ay_pre(i, j))/2.0d0)

                    E_v(i, j) = AYM*v_pre(i,j-1) + AXM*v_pre(i-1,j) - A0*v_pre(i, j)  + AXP*v_pre(i+1,j) + AYP*v_pre(i,j+1) - RHS_v(i, j)
                    v_pre(i, j) = v_pre(i, j) + omg * E_v(i, j) / A0
                    er = er + sqrt(E_v(i, j) ** 2)
                enddo
            enddo
    ...
    end subroutine prespeed_cal_implicit

    subroutine Poisson_equation(u_pre, v_pre, phi) ! SOR法を用いてポアソン方程式を計算
...
program main
    ...
    real(8), allocatable :: Ax(:, :), Ax_pre(:, :) ! 対流項
    real(8), allocatable :: Ay(:, :), Ay_pre(:, :) 
    real(8), allocatable :: Bx(:, :) ! 粘性項
    real(8), allocatable :: By(:, :)
    real(8), allocatable :: Fx(:, :), Fy(:, :) ! 外力項
    ...
    allocate(Ax_pre(1:Nx, 1:Ny))
    allocate(Ay(1:Nx, 1:Ny))
    allocate(Ay_pre(1:Nx, 1:Ny))

    allocate(Bx(1:Nx, 1:Ny))
    allocate(By(1:Nx, 1:Ny))

    allocate(Fx(1:Nx, 1:Ny))
    allocate(Fy(1:Nx, 1:Ny))
    ...
    ! １ステップ目はオイラー陽解法
    call prespeed_cal_explicit(Ax, Ay, Bx, By, u, u_pre, v, v_pre, dpdx, dpdy, p, Fx, Fy)
    ...
    call Poisson_equation(u_pre, v_pre, phi) ! 圧力ポアソン方程式を解く
    ...
    do k = 2, steps
        ...
        ! 陰解法で予測計算
        call prespeed_cal_implicit(Ax, Ay, Ax_pre, Ay_pre, Bx, By, u, u_pre, v, v_pre, dpdx, dpdy, p, Fx, Fy) ! 予測速度計算

        ! 過去の情報を保存
        Ax_pre(:, :) = Ax(:, :)
        Ay_pre(:, :) = Ay(:, :)

        ! (境界条件は陰解法サブルーチンの中に記述済み)
